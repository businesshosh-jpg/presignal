...
def main():
    ...
    for path, value in [(out / "slice_009_manifest.json", manifest), (out / "population_proof.json", proof), (out / "exclusion_proof.json", proof), (out / "manifest_decision.json", decision), (auth_out / "authorization.json", auth), (auth_out / "authorization_validation.json", {"decision": "SLICE_009_END_TO_END_EXECUTION_AUTHORIZED_NOT_STARTED", "external_access": 0, "manifest_id": manifest_id, "manifest_fingerprint": manifest["manifest_fingerprint"], "authorization_fingerprint": auth["authorization_fingerprint"]})]:
