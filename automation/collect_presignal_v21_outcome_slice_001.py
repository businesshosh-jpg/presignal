...
def preflight() -> tuple[dict[str, Any], list[dict[str, Any]], Path]:
    ...
    if len(episodes) != 12 or len({row["episode_id"] for row in episodes}) != 12:
    ...
    if days != ["2024-05-01", "2024-05-02", "2024-05-03"]:
    ...
    existing = list(OUTPUT_ROOT.glob("PPHB-R1-OUTCOME-COLLECTION-SLICE-001-*"))
    for prior in existing:
        decision_path = prior / "collection_decision.json"
        prior_decision = read_json(decision_path) if decision_path.exists() else {}
        if prior_decision.get("collection_decision") != "OUTCOME_COLLECTION_SLICE_001_BLOCKED":
            raise RuntimeError("OUTCOME_COLLECTION_ALREADY_EXISTS")
    run_id = f"PPHB-R1-OUTCOME-COLLECTION-SLICE-001-{RUN_STAMP}-{manifest_hash[-12:]}"
...
def collect(manifest: dict[str, Any], episodes: list[dict[str, Any]], run_dir: Path) -> None:
    ...
    for day in sorted(by_day):
        ...
        request_id = "OUTCOME_SLICE_001_DAY_" + day.replace("-", "")
