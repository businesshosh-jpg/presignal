#!/usr/bin/env python3
"""Execute or fail-closed ATTN_BATCH_005 from the frozen Attention execution plan."""
...
BATCH_ID = "ATTN_BATCH_005"
EXPECTED_CALL_COUNT = 12
EXPECTED_START_HEAD = "ca7000365c1877ef660c642fb1b7c54408f5c5ff"
...
def load_batch_calls() -> list[dict[str, Any]]:
    ...
    if len(calls) != EXPECTED_CALL_COUNT:
        raise batch004.base.AttentionBatchError("BATCH_005_CALL_COUNT_MISMATCH")
    prior_ids: set[str] = set()
    for prior_batch_id in ("ATTN_BATCH_001", "ATTN_BATCH_002", "ATTN_BATCH_003", "ATTN_BATCH_004"):
    ...
    if overlap:
        raise batch004.base.AttentionBatchError("BATCH_005_OVERLAPS_PRIOR_BATCH:" + ",".join(overlap))
    return calls
...
def patched_batch004() -> Any:
    ...
    batch004.PRIOR_BATCH_IDS = ("ATTN_BATCH_001", "ATTN_BATCH_002", "ATTN_BATCH_003", "ATTN_BATCH_004")
    batch004.RUN_ID_PREFIX = "PPHB-R1-ATTENTION-EXECUTION-BATCH-005-"
    batch004.EXECUTION_STATUS_COMPLETE = "ATTENTION_BATCH_005_COMPLETE"
    batch004.EXECUTION_STATUS_PARTIAL = "ATTENTION_BATCH_005_PARTIALLY_COMPLETE"
    batch004.EXECUTION_STATUS_BLOCKED = "ATTENTION_BATCH_005_BLOCKED"
    batch004.SCALING_READY = "READY_FOR_ATTENTION_BATCH_006"
    batch004.SCALING_REPAIR = "REPAIR_BEFORE_BATCH_006"
    batch004.SCALING_RETRY = "RETRY_FAILED_BATCH_005_CALLS_REQUIRES_AUTHORIZATION"
    batch004.FAILED_CALL_RETRY_RECOMMENDATION = "NO_AUTOMATIC_RETRY_IN_BATCH_005"
    try:
        yield
    finally:
...
def patch_run_metadata(run_dir: Path) -> None:
    ...
    batch_contract_path = run_dir / "batch_execution_contract.json"
    governing_manifest_path = run_dir / "governing_artifact_manifest.json"

    if run_manifest_path.exists():
        manifest = batch004.read_json(run_manifest_path)
        manifest["governing_batch_004_execution_id"] = GOVERNING_BATCH_004_EXECUTION_ID
        batch004.write_json(run_manifest_path, manifest)
    if batch_contract_path.exists():
        contract = batch004.read_json(batch_contract_path)
        contract["governing_batch_004_execution_id"] = GOVERNING_BATCH_004_EXECUTION_ID
        batch004.write_json(batch_contract_path, contract)
    if governing_manifest_path.exists():
        manifest = batch004.read_json(governing_manifest_path)
        manifest["governing_batch_004_execution_id"] = GOVERNING_BATCH_004_EXECUTION_ID
        batch004.write_json(governing_manifest_path, manifest)
