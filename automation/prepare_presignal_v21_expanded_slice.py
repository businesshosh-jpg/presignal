...
SLICE = "SLICE-011"
MANIFEST_TIME = "20260803T220000Z"
AUTH_TIME = "20260803T230000Z"
MAX_EPISODES = 36
...
def main():
    ...
    if population != {"valid_forecasts": 130, "pack_a": 65, "pack_e": 65, "complete_pack_a_e_pairs": 65, "pack_a_only": 0, "pack_e_only": 0, "unpaired": 0} or len(days) != 14:
    ...
    for path, value in [(out / "slice_011_manifest.json", manifest), (out / "population_proof.json", proof), (out / "exclusion_proof.json", proof), (out / "manifest_decision.json", decision), (auth_out / "authorization.json", auth), (auth_out / "authorization_validation.json", {"decision": "SLICE_011_END_TO_END_EXECUTION_AUTHORIZED_NOT_STARTED", "external_access": 0, "manifest_id": manifest_id, "manifest_fingerprint": manifest["manifest_fingerprint"], "authorization_fingerprint": auth["authorization_fingerprint"]})]:
        path.write_text(json.dumps(value, indent=2, sort_keys=True) + "\n")
    print(out / "slice_011_manifest.json"); print(auth_out / "authorization.json"); print(manifest["manifest_fingerprint"]); print(auth["authorization_fingerprint"])
