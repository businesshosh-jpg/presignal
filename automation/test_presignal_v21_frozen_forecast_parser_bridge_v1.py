...
from automation import run_presignal_v21_single_event_path_pair_v1 as frozen
from automation.test_run_presignal_v21_single_event_path_pair_v1 import response


PARSER = Path(frozen.__file__)
PARSER_SHA256 = "dd3e18ed113575bcc0036a4f7b56bc436ea1a360f40faa6f5d41662699a01287"
...
class FrozenForecastParserBridgeTests(unittest.TestCase):
    ...
    def test_parser_source_and_manifest_binding_remain_valid(self):
        ...
        self.assertEqual(binding.gate()["status"], "R3_RUNTIME_BINDING_VALIDATED")

    def test_directional_and_no_signal_payloads_are_preserved(self):
        directional = frozen.parse_provider_output(response())
        no_signal = dict(response(), no_signal_flag=True, expected_initial_direction="UNCERTAIN", path=[])
