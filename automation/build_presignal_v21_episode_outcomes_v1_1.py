...
OUTPUT = ROOT / "outputs" / "presignal_v21_episode_outcomes_v1_1"
...
def _signed_pips(observation, anchor_price):
    return round((observation["close"] - anchor_price) / 0.01, 2)


def _is_minute_aligned(value):
    instant = utc(value)
    return instant.second == 0 and instant.microsecond == 0


def immediate_impulse_sidecar(observations, release, anchor_price):
    window_end = release + timedelta(seconds=contract.IMMEDIATE_IMPULSE_WINDOW_SECONDS_DEFAULT)
    window = [item for item in observations.between(release + timedelta(microseconds=1), window_end)]
    base = {
    ...
    if not window:
        base["immediate_impulse_outcome_state"] = "UNAVAILABLE_UNSUPPORTED_RESOLUTION"
        return base
    subminute = any(not _is_minute_aligned(item["timestamp"]) for item in window)
    base["immediate_impulse_outcome_state"] = "SUPPORTED" if subminute else "APPROXIMATION_ONLY"
    directional = []
    for item in window:
        pips = _signed_pips(item, anchor_price)
        direction = contract.direction_for_pips(pips)
        if direction in {"UP", "DOWN"}:
            directional.append({"timestamp": item["timestamp"], "pips": pips, "direction": direction})
    if not directional:
        base.update({
            "first_meaningful_excursion_direction": "FLAT",
            "first_meaningful_excursion_pips": 0.0,
            "confirmed_initial_direction": "FLAT",
            "initial_peak_pips": 0.0,
            "maximum_opposite_excursion_pips": 0.0,
        })
        return base
    first = directional[0]
    confirmed = directional[-1]
    false_initial = first["direction"] != confirmed["direction"]
    confirmed_direction = confirmed["direction"]
    same_direction = [item for item in directional if item["direction"] == confirmed_direction]
    if confirmed_direction == "UP":
        initial_peak = max(same_direction, key=lambda item: (item["pips"], -utc(item["timestamp"]).timestamp()))
        opposite_excursion = min([item["pips"] for item in directional if item["direction"] == "DOWN"], default=0.0)
    else:
        initial_peak = min(same_direction, key=lambda item: (item["pips"], utc(item["timestamp"]).timestamp()))
        opposite_excursion = max([item["pips"] for item in directional if item["direction"] == "UP"], default=0.0)
    base.update({
        "first_meaningful_excursion_direction": first["direction"],
        "first_meaningful_excursion_timestamp": first["timestamp"],
        "first_meaningful_excursion_pips": first["pips"],
        "confirmed_initial_direction": confirmed_direction,
        "initial_direction_confirmation_timestamp": confirmed["timestamp"],
        "initial_peak_pips": initial_peak["pips"],
        "initial_peak_timestamp": initial_peak["timestamp"],
        "maximum_opposite_excursion_pips": opposite_excursion,
        "false_initial_excursion_flag": false_initial,
        "false_initial_excursion_direction": first["direction"] if false_initial else None,
    })
    return base
...
def outcome_record(episode, observations, request_lineage, intervening, acquisition_ts):
    ...
    base.update(immediate_impulse_sidecar(observations, release, anchor["close"]))
    ...
    if base["confirmed_initial_direction"] in {"UP", "DOWN"} and base["initial_peak_pips"] not in (None, 0):
        base["initial_peak_retention_at_t5"] = round(base["pips_5m"] / base["initial_peak_pips"], 6)
