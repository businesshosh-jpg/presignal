...
IMMEDIATE_IMPULSE_OUTCOME_STATES = {
    "SUPPORTED",
    "APPROXIMATION_ONLY",
    "UNAVAILABLE_UNSUPPORTED_RESOLUTION",
    "OUTCOME_UNAVAILABLE",
    "INVALID_MARKET_DATA",
    "NOT_AVAILABLE_LEGACY_SCHEMA",
}
IMMEDIATE_IMPULSE_DIRECTION_RESULTS = {"CORRECT", "INCORRECT", "OUTCOME_UNAVAILABLE", "NOT_APPLICABLE"}
IMMEDIATE_IMPULSE_METHOD_VERSION = "presignal_immediate_impulse_v1"
IMMEDIATE_IMPULSE_WINDOW_SECONDS_DEFAULT = 120
...
EVALUATION_FIELDS = [
    "object", "schema_version", "system_version", "evaluation_id", "run_id", "prediction_id", "outcome_id", "episode_id", "provider", "model", "information_arm", "immediate_impulse_direction_result", "immediate_impulse_peak_range_covered", "immediate_impulse_peak_range_distance_error_pips", "immediate_impulse_peak_midpoint_absolute_error_pips", "false_initial_excursion_observed", "direction_5m_ok", "t5_range_covered", "t5_range_distance_error_pips", "t5_midpoint_absolute_error_pips", "initial_peak_retention_at_t5", "initial_impulse_sustained_to_t5", "initial_impulse_faded_by_t5", "initial_impulse_reversed_by_t5", "direction_15m_ok", "direction_30m_ok", "direction_60m_ok", "magnitude_15m_error", "reversal_ok", "no_signal_ok", "primary_endpoint_name", "primary_endpoint_value", "overall_path_score", "evaluation_note", "evaluation_contract_version", "evaluation_fingerprint", "generated_ts", "status", "error_message",
]
...
def validate_prediction(record: Mapping[str, Any]) -> None:
    ...
    if record["no_signal_flag"]:
        ...
        _require(record["immediate_impulse_peak_pips_min"] is None and record["immediate_impulse_peak_pips_max"] is None, "PREDICTION_PROVIDER_ERROR_IMMEDIATE_RANGE")
        _require(record["immediate_impulse_confidence"] is None and record["immediate_impulse_window_seconds"] is None, "PREDICTION_PROVIDER_ERROR_IMMEDIATE_META")
        ...
        _require(record["immediate_impulse_direction"] in DIRECTIONS - {"UNCERTAIN"}, "PREDICTION_IMMEDIATE_DIRECTION")
        immediate_low = _number(record["immediate_impulse_peak_pips_min"], "PREDICTION_IMMEDIATE_PIPS_MIN", 0)
        immediate_high = _number(record["immediate_impulse_peak_pips_max"], "PREDICTION_IMMEDIATE_PIPS_MAX", 0)
        _require(immediate_low <= immediate_high, "PREDICTION_IMMEDIATE_PIP_RANGE")
        _number(record["immediate_impulse_confidence"], "PREDICTION_IMMEDIATE_CONFIDENCE", 0, 1)
        _require(record["immediate_impulse_window_seconds"] == IMMEDIATE_IMPULSE_WINDOW_SECONDS_DEFAULT, "PREDICTION_IMMEDIATE_WINDOW")
        _require(record["early_reaction_5m_direction"] in DIRECTIONS - {"UNCERTAIN"}, "PREDICTION_DIRECTION")
...
def validate_prediction_path_transaction(prediction: Mapping[str, Any], paths: Sequence[Mapping[str, Any]]) -> None:
    ...
    _require(prediction["early_reaction_5m_direction"] == paths[0]["expected_direction"], "PREDICTION_EARLY_REACTION_BINDING")
...
def validate_outcome(record: Mapping[str, Any]) -> None:
    ...
    if record["status"] == "UNAVAILABLE":
        _require(record["immediate_impulse_outcome_state"] in {"OUTCOME_UNAVAILABLE", "UNAVAILABLE_UNSUPPORTED_RESOLUTION", "INVALID_MARKET_DATA"}, "OUTCOME_UNAVAILABLE_IMMEDIATE_STATE")
        _require(record["immediate_impulse_method_version"] == IMMEDIATE_IMPULSE_METHOD_VERSION, "OUTCOME_IMMEDIATE_METHOD_VERSION")
        _require(record["immediate_impulse_window_seconds"] == IMMEDIATE_IMPULSE_WINDOW_SECONDS_DEFAULT, "OUTCOME_IMMEDIATE_WINDOW")
        _require(record["first_meaningful_excursion_direction"] == "UNAVAILABLE", "OUTCOME_UNAVAILABLE_FIRST_DIRECTION")
        _require(record["confirmed_initial_direction"] == "UNAVAILABLE", "OUTCOME_UNAVAILABLE_CONFIRMED_DIRECTION")
        _require(isinstance(record["false_initial_excursion_flag"], bool), "OUTCOME_UNAVAILABLE_FALSE_EXCURSION_FLAG")
        for key in ("anchor_price_ts", "anchor_price", "first_meaningful_excursion_timestamp", "first_meaningful_excursion_pips", "initial_direction_confirmation_timestamp", "initial_peak_pips", "initial_peak_timestamp", "maximum_opposite_excursion_pips", "false_initial_excursion_direction", "initial_peak_retention_at_t5", "price_5m", "price_15m", "price_30m", "price_60m", "pips_5m", "pips_15m", "pips_30m", "pips_60m", "max_up_pips", "max_down_pips", "max_up_ts", "max_down_ts", "initial_direction", "reversal_ts"):
        ...
        _require(record["immediate_impulse_outcome_state"] in {"SUPPORTED", "APPROXIMATION_ONLY", "UNAVAILABLE_UNSUPPORTED_RESOLUTION", "NOT_AVAILABLE_LEGACY_SCHEMA"}, "OUTCOME_IMMEDIATE_STATE")
        _require(record["immediate_impulse_method_version"] == IMMEDIATE_IMPULSE_METHOD_VERSION, "OUTCOME_IMMEDIATE_METHOD_VERSION")
        _require(record["immediate_impulse_window_seconds"] == IMMEDIATE_IMPULSE_WINDOW_SECONDS_DEFAULT, "OUTCOME_IMMEDIATE_WINDOW")
        _require(isinstance(record["false_initial_excursion_flag"], bool), "OUTCOME_FALSE_INITIAL_EXCURSION")
        ...
        if record["immediate_impulse_outcome_state"] in {"UNAVAILABLE_UNSUPPORTED_RESOLUTION", "NOT_AVAILABLE_LEGACY_SCHEMA"}:
            _require(record["first_meaningful_excursion_direction"] == "UNAVAILABLE", "OUTCOME_LEGACY_FIRST_EXCURSION_DIRECTION")
            _require(record["confirmed_initial_direction"] == "UNAVAILABLE", "OUTCOME_LEGACY_CONFIRMED_DIRECTION")
            for key in ("first_meaningful_excursion_timestamp", "first_meaningful_excursion_pips", "initial_direction_confirmation_timestamp", "initial_peak_pips", "initial_peak_timestamp", "maximum_opposite_excursion_pips", "false_initial_excursion_direction", "initial_peak_retention_at_t5"):
                _require(record[key] is None, "OUTCOME_LEGACY_IMMEDIATE_VALUE:" + key)
            _require(record["false_initial_excursion_flag"] is False, "OUTCOME_LEGACY_FALSE_EXCURSION_FLAG")
        else:
            _require(record["first_meaningful_excursion_direction"] in OUTCOME_DIRECTIONS - {"UNAVAILABLE"}, "OUTCOME_FIRST_EXCURSION_DIRECTION")
            _require(record["confirmed_initial_direction"] in OUTCOME_DIRECTIONS - {"UNAVAILABLE"}, "OUTCOME_CONFIRMED_INITIAL_DIRECTION")
            if record["first_meaningful_excursion_direction"] == "FLAT":
                _require(record["first_meaningful_excursion_timestamp"] is None, "OUTCOME_FLAT_FIRST_EXCURSION_TS")
                _require(round(_number(record["first_meaningful_excursion_pips"], "OUTCOME_FLAT_FIRST_EXCURSION_PIPS"), 8) == 0.0, "OUTCOME_FLAT_FIRST_EXCURSION_PIPS")
            else:
                _timestamp(record["first_meaningful_excursion_timestamp"], "OUTCOME_FIRST_EXCURSION_TS")
                _number(record["first_meaningful_excursion_pips"], "OUTCOME_FIRST_EXCURSION_PIPS")
            if record["confirmed_initial_direction"] == "FLAT":
                _require(record["initial_direction_confirmation_timestamp"] is None, "OUTCOME_FLAT_CONFIRMATION_TS")
                _require(round(_number(record["initial_peak_pips"], "OUTCOME_FLAT_INITIAL_PEAK_PIPS"), 8) == 0.0, "OUTCOME_FLAT_INITIAL_PEAK_PIPS")
                _require(record["initial_peak_timestamp"] is None, "OUTCOME_FLAT_INITIAL_PEAK_TS")
                _require(round(_number(record["maximum_opposite_excursion_pips"], "OUTCOME_FLAT_MAX_OPPOSITE_EXCURSION"), 8) == 0.0, "OUTCOME_FLAT_MAX_OPPOSITE_EXCURSION")
                _require(record["initial_peak_retention_at_t5"] is None, "OUTCOME_FLAT_RETENTION")
            else:
                _timestamp(record["initial_direction_confirmation_timestamp"], "OUTCOME_CONFIRMATION_TS")
                _number(record["initial_peak_pips"], "OUTCOME_INITIAL_PEAK_PIPS")
                _timestamp(record["initial_peak_timestamp"], "OUTCOME_INITIAL_PEAK_TS")
                _number(record["maximum_opposite_excursion_pips"], "OUTCOME_MAX_OPPOSITE_EXCURSION")
                if record["initial_peak_retention_at_t5"] is not None:
                    _number(record["initial_peak_retention_at_t5"], "OUTCOME_INITIAL_PEAK_RETENTION")
            if record["false_initial_excursion_flag"]:
                _require(record["false_initial_excursion_direction"] in {"UP", "DOWN"}, "OUTCOME_FALSE_EXCURSION_DIRECTION")
            else:
                _require(record["false_initial_excursion_direction"] is None, "OUTCOME_FALSE_EXCURSION_DIRECTION_ABSENT")
...
def _midpoint_absolute_error(realized_abs_pips: float, low: float, high: float) -> float:
    return abs(realized_abs_pips - ((low + high) / 2.0))
...
def validate_evaluation(record: Mapping[str, Any], prediction: Mapping[str, Any], outcome: Mapping[str, Any], paths: Sequence[Mapping[str, Any]] | None = None) -> None:
    ...
    if unavailable:
        ...
        _require(record["immediate_impulse_direction_result"] == "OUTCOME_UNAVAILABLE", "EVALUATION_UNAVAILABLE_IMMEDIATE_RESULT")
        _require(record["immediate_impulse_peak_range_covered"] is None and record["immediate_impulse_peak_range_distance_error_pips"] is None and record["immediate_impulse_peak_midpoint_absolute_error_pips"] is None, "EVALUATION_UNAVAILABLE_IMMEDIATE_METRICS")
        _require(record["false_initial_excursion_observed"] is None, "EVALUATION_UNAVAILABLE_FALSE_INITIAL_EXCURSION")
        ...
        _require(record["immediate_impulse_direction_result"] == "NOT_APPLICABLE", "EVALUATION_NO_SIGNAL_IMMEDIATE_RESULT")
        ...
        if outcome["immediate_impulse_outcome_state"] == "SUPPORTED":
            expected_result = "CORRECT" if prediction["immediate_impulse_direction"] == outcome["confirmed_initial_direction"] else "INCORRECT"
            _require(record["immediate_impulse_direction_result"] == expected_result, "EVALUATION_IMMEDIATE_DIRECTION_RESULT")
            realized_impulse_abs = abs(_number(outcome["initial_peak_pips"], "EVALUATION_IMMEDIATE_REALIZED_PIPS"))
            impulse_low = _number(prediction["immediate_impulse_peak_pips_min"], "EVALUATION_IMMEDIATE_PRED_LOW", 0)
            impulse_high = _number(prediction["immediate_impulse_peak_pips_max"], "EVALUATION_IMMEDIATE_PRED_HIGH", 0)
            _require(record["immediate_impulse_peak_range_covered"] is (impulse_low <= realized_impulse_abs <= impulse_high), "EVALUATION_IMMEDIATE_RANGE_COVERED")
            _require(round(_number(record["immediate_impulse_peak_range_distance_error_pips"], "EVALUATION_IMMEDIATE_RANGE_DISTANCE", 0), 8) == round(_interval_error(realized_impulse_abs, impulse_low, impulse_high), 8), "EVALUATION_IMMEDIATE_RANGE_DISTANCE_VALUE")
            _require(round(_number(record["immediate_impulse_peak_midpoint_absolute_error_pips"], "EVALUATION_IMMEDIATE_MIDPOINT_DISTANCE", 0), 8) == round(_midpoint_absolute_error(realized_impulse_abs, impulse_low, impulse_high), 8), "EVALUATION_IMMEDIATE_MIDPOINT_DISTANCE_VALUE")
        else:
            _require(record["immediate_impulse_direction_result"] == "NOT_APPLICABLE", "EVALUATION_IMMEDIATE_NOT_APPLICABLE")
            _require(record["immediate_impulse_peak_range_covered"] is None and record["immediate_impulse_peak_range_distance_error_pips"] is None and record["immediate_impulse_peak_midpoint_absolute_error_pips"] is None, "EVALUATION_IMMEDIATE_APPROXIMATION_METRICS")
        _require(record["false_initial_excursion_observed"] is outcome["false_initial_excursion_flag"], "EVALUATION_FALSE_INITIAL_EXCURSION")
        correctness = []
        for horizon in HORIZONS:
        ...
        realized_5m_abs = abs(outcome["pips_5m"])
        path5 = by_horizon[5]
        _require(record["t5_range_covered"] is (path5["expected_pips_min"] <= realized_5m_abs <= path5["expected_pips_max"]), "EVALUATION_T5_RANGE_COVERED")
        _require(round(_number(record["t5_range_distance_error_pips"], "EVALUATION_T5_RANGE_DISTANCE", 0), 8) == round(_interval_error(realized_5m_abs, path5["expected_pips_min"], path5["expected_pips_max"]), 8), "EVALUATION_T5_RANGE_DISTANCE_VALUE")
        _require(round(_number(record["t5_midpoint_absolute_error_pips"], "EVALUATION_T5_MIDPOINT_DISTANCE", 0), 8) == round(_midpoint_absolute_error(realized_5m_abs, path5["expected_pips_min"], path5["expected_pips_max"]), 8), "EVALUATION_T5_MIDPOINT_DISTANCE_VALUE")
        _require(record["initial_peak_retention_at_t5"] == outcome["initial_peak_retention_at_t5"], "EVALUATION_T5_RETENTION")
        confirmed = outcome["confirmed_initial_direction"]
        if confirmed in {"UP", "DOWN"} and outcome["initial_peak_retention_at_t5"] is not None:
            opposite = {"UP": "DOWN", "DOWN": "UP"}[confirmed]
            sustained = outcome["direction_5m"] == confirmed
            reversed_flag = outcome["direction_5m"] == opposite
            faded = not sustained and not reversed_flag
            _require(record["initial_impulse_sustained_to_t5"] is sustained, "EVALUATION_T5_SUSTAINED")
            _require(record["initial_impulse_reversed_by_t5"] is reversed_flag, "EVALUATION_T5_REVERSED")
            _require(record["initial_impulse_faded_by_t5"] is faded, "EVALUATION_T5_FADED")
        else:
            _require(record["initial_impulse_sustained_to_t5"] is None and record["initial_impulse_reversed_by_t5"] is None and record["initial_impulse_faded_by_t5"] is None, "EVALUATION_T5_CLASSIFICATIONS_NULL")
