#!/usr/bin/env python3
"""Freeze the corrected, single-use Slice 003 authorization without execution."""
...
BLOCKED_AUTH_ID = "PPHB-R1-OUTCOME-SLICE-003-END-TO-END-AUTHORIZATION-20260803T160000Z"
BLOCKED_AUTH_FINGERPRINT = "sha256:42faad88745d71b047fb1136b5f799b45501bed621874b85a894f6c2def36eb8"
AUTHORIZATION_ID = "PPHB-R1-OUTCOME-SLICE-003-REPLACEMENT-END-TO-END-AUTHORIZATION-20260803T171000Z"
...
def build_authorization(controller_commit: str) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    days = sorted({row["release_ts"][:10] for row in rows})
    population = manifest["authorized_forecast_population"]
    identities = [row["episode_id"] for row in rows]
    collection_ids = [row["outcome_collection_identity"]["duplicate_prevention_identity"] for row in rows]
    auth = {
        "authorization_id": AUTHORIZATION_ID,
        "authorization_schema_version": "AUTHORIZED_SLICE_END_TO_END_AUTH_V1",
        ...
        "ceilings": {
            "max_apps_script_reads": len(days),
            "max_market_data_attempts": 12,
            "max_total_external_requests": len(days) + 12,
            "google_write_ceiling": 0,
            "max_attachment_records": len(rows),
            "max_evaluation_artifacts": 1,
        },
        "attachment_write_ceiling": {"google_writes": 0, "local_append_only_records": len(rows)},
        ...
        "required_collection_artifact": {
            "stage": "collection",
            "required_files": ["collection_reconciliation.json", "candidate_outcomes.jsonl"],
            "manifest_sha256": manifest["manifest_fingerprint"],
        },
        "evaluation_population_rule": f"{population['valid_forecasts']} authoritative valid forecasts mapped one-to-one to {len(rows)} attached Slice 003 Outcomes; terminal-invalid excluded; Pack A/E remain distinct",
        "evaluation_population": population,
        "permitted_metrics": [
            "T+15 directional accuracy", "Immediate Impulse directional accuracy",
            "magnitude or pip error", "horizon accuracy", "path accuracy", "reversal accuracy",
        ],
        ...
        "supersedes_authorization_id": BLOCKED_AUTH_ID,
        "supersedes_authorization_fingerprint": BLOCKED_AUTH_FINGERPRINT,
        "supersession_reason": "Blocked authorization allowed 3 Apps Script reads for a population spanning 10 UTC release days; canonical collection requires one read per distinct day.",
        "timestamp_classification": "DETERMINISTIC_ARTIFACT_NAMING_METADATA",
        "live_stop_state_before_execution": "SLICE_003_REPLACEMENT_END_TO_END_EXECUTION_AUTHORIZED_NOT_STARTED",
    }
    auth["authorization_fingerprint"] = fingerprint(auth)
    proof_out = {"release_days_utc": days, "distinct_release_day_count": len(days), "episode_to_release_day": {row["episode_id"]: row["release_ts"][:10] for row in rows}, "canonical_rule": "one Apps Script source read per distinct UTC release day", "duplicate_day_reads": 0, "required_apps_script_reads": len(days), "required_market_data_attempts": 12, "required_total_external_requests": len(days) + 12}
    return auth, proof_out
...
def main() -> None:
    ...
    auth, day_proof = build_authorization(args.controller_commit)
    suffix = auth["authorization_fingerprint"].split(":", 1)[1][:20]
    output = BASE / f"PPHB-R1-OUTCOME-SLICE-003-REPLACEMENT-END-TO-END-AUTHORIZATION-20260803T171000Z-{suffix}"
    if output.exists():
        raise SystemExit("OUTPUT_ALREADY_EXISTS")
    output.mkdir(parents=True)
    write(output / "authorization.json", auth)
    write(output / "day_ceiling_proof.json", {**day_proof, "manifest_id": auth["manifest_id"], "manifest_fingerprint": auth["manifest_fingerprint"], "external_access": 0})
    write(output / "timestamp_preflight.json", {"authorization_id": auth["authorization_id"], "classification": auth["timestamp_classification"], "validity_boundary": "not an issuance or future-effective time", "decision": "TIMESTAMP_AUTHORITY_CONFIRMED", "external_access": 0})
    write(output / "blocked_authorization_rejection.json", {"blocked_authorization_id": BLOCKED_AUTH_ID, "blocked_authorization_fingerprint": BLOCKED_AUTH_FINGERPRINT, "decision": "AUTHORIZATION_NON_REUSABLE_BLOCKED", "reason": "recorded governance-blocked evidence is non-reusable", "external_access": 0})
    write(output / "authorization_decision.json", {"decision": "SLICE_003_REPLACEMENT_END_TO_END_AUTHORIZATION_FROZEN", "readiness": "SLICE_003_REPLACEMENT_END_TO_END_EXECUTION_READY", "live_state": auth["live_stop_state_before_execution"], "authorization_id": auth["authorization_id"], "authorization_fingerprint": auth["authorization_fingerprint"], "manifest_id": auth["manifest_id"], "manifest_fingerprint": auth["manifest_fingerprint"], "external_access": 0})
    write(output / "run_manifest.json", {"run_id": output.name, "move": "REPLACE_BLOCKED_SLICE_003_AUTHORIZATION_WITH_DAY_BOUND_CEILINGS", "authorization_id": auth["authorization_id"], "authorization_fingerprint": auth["authorization_fingerprint"], "manifest_id": auth["manifest_id"], "manifest_fingerprint": auth["manifest_fingerprint"], "controller_commit": args.controller_commit, "provider_calls": 0, "google_reads": 0, "market_data_calls": 0, "google_writes": 0, "attachment": 0, "evaluation": 0, "append_only": True})
    print(output)
