...
def load_frozen_package(package: Path = PACKAGE) -> dict[str, Any]:
    ...
    state = {
        ...
        "attention": rows(snapshot / "authoritative_attention_map.jsonl") if (snapshot / "authoritative_attention_map.jsonl").exists() else [],
        "attention_available": (snapshot / "authoritative_attention_map.jsonl").exists(),
    }
    ...
    for episode in sorted(episodes, key=lambda row: row["episode_id"]):
        ...
        if not attention:
            ...
            for provider_model in sorted({(row.get("provider"),row.get("model")) for row in attention}):
                records=[row for row in attention if (row.get("provider"),row.get("model"))==provider_model and row.get("event_id") in episode["member_event_ids"]]
            ...
            attention_rows.append({"episode_id":episode_id,"source_session_id":session_id,"status":"COMPATIBLE","reason":"","records":selected})
...
def run(mode: str, output: Path = OUTPUT) -> dict[str, Any]:
    ...
    state=load_frozen_package(); episodes=rows(EPISODES); roles=rows(ROLES)
    if mode=="mock":
    ...
    manifest={"decision":"V2_1_STEP5_V2_INFORMATION_INFRASTRUCTURE_REUSE_VALIDATED" if counts["fully_step_6_ready_episodes"] else "V2_1_STEP5_TARGETED_COMPATIBILITY_REPAIR_REQUIRED","mode":mode,"package":state["package"],"counts":counts,"unavailable_by_reason":dict(sorted(unavailable.items())),"pack_a_input_fingerprint":fingerprint(result["pack_a"]),"pack_e_input_fingerprint":fingerprint(result["pack_e"]),"attention_export_available":state["attention_available"],"external_calls":{"provider":0,"acquisition":0,"market_data":0,"apps_script":0,"google_sheets_writes":0}}
...
def main() -> None:
    parser=argparse.ArgumentParser(); parser.add_argument("--mode",choices=("inventory","compatibility","mock"),default="compatibility"); parser.add_argument("--output",type=Path,default=OUTPUT); args=parser.parse_args()
    print(json.dumps(run(args.mode,args.output),sort_keys=True))
