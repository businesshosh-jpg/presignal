...
class ExecuteAttentionBatch005Test(unittest.TestCase):
    def test_only_batch_005_calls_are_loaded(self) -> None:
        calls = batch005.load_batch_calls()
    ...
    def test_provider_model_assignments_remain_frozen(self) -> None:
        ...
        self.assertTrue(all(row["model"] == batch005.batch004.base.PROVIDER_MODELS[row["provider"]] for row in calls))

    def test_manifest_transport_agreement_required(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            dispatcher = AuthorityConflictDispatcher()
            result = batch005.execute_batch(
                output_root=Path(tmp),
                dispatcher=dispatcher,
                fixed_timestamp="2026-07-29T07:00:00Z",
    ...
    def test_expected_and_returned_event_sets_must_match(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            rows = batch005.batch004.read_jsonl(result["run_dir"] / "event_completeness_results.jsonl")
    ...
    def test_raw_output_preserved_and_successful_calls_cannot_repeat(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            raw = batch005.batch004.read_jsonl(first["run_dir"] / "raw_provider_outputs.jsonl")
            self.assertEqual(len(raw), 12)
            second = batch005.execute_batch(
                output_root=Path(tmp),
                dispatcher=dispatcher,
                fixed_timestamp="2026-07-29T07:00:00Z",
    ...
    def test_only_complete_validated_results_enter_normalized_ledger(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            normalized = batch005.batch004.read_jsonl(result["run_dir"] / "normalized_attention_results.jsonl")
    ...
    def test_blocked_preflight_fails_closed_before_provider_calls(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            with patch.object(batch005.batch004, "preflight_credentials", side_effect=GoogleCredentialError("GOOGLE_OAUTH_TOKEN_MISSING", "missing token")):
                result = batch005.execute_batch(output_root=Path(tmp), fixed_timestamp="2026-07-29T07:00:00Z", enforce_head=False)
            self.assertEqual(result["decision"]["execution_status"], "ATTENTION_BATCH_005_BLOCKED")
            self.assertEqual(result["reconciliation"]["attempted_calls"], 0)

    def test_no_pack_no_forecast_no_batch_006_execution(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            dispatcher = CompletenessDispatcher()
            result = batch005.execute_batch(
                output_root=Path(tmp),
            ...
            manifest = batch005.batch004.base.read_json(result["run_dir"] / "run_manifest.json")
            self.assertEqual(manifest["pack_construction_executed"], 0)
            self.assertEqual(manifest["forecast_calls_executed"], 0)
            self.assertEqual(result["decision"]["scaling_decision"], "READY_FOR_ATTENTION_BATCH_006")
