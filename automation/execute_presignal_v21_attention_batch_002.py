#!/usr/bin/env python3
"""Execute or fail-closed ATTN_BATCH_002 from the frozen Attention execution plan."""
...
def load_batch_calls() -> list[dict[str, Any]]:
    ...
    if len(calls) != EXPECTED_CALL_COUNT:
        raise base.AttentionBatchError("BATCH_002_CALL_COUNT_MISMATCH")
    batch_001_ids = {row["call_id"] for row in base.load_batch_calls(base.BATCH_ID)}
    overlap = [row["call_id"] for row in calls if row["call_id"] in batch_001_ids]
    if overlap:
        raise base.AttentionBatchError("BATCH_002_OVERLAPS_BATCH_001:" + ",".join(overlap))
    return calls
...
def materialize_run(
    *,
    ...
    run_id = (
        "PPHB-R1-ATTENTION-EXECUTION-BATCH-002-"
        + ts.replace(":", "").replace("-", "")
        + "-"
...
def initialize_run_files(
    ...
    write_json(
        run_dir / "batch_execution_contract.json",
        {
            ...
            "identity_normalization_rules": "existing validated provider-identity normalization only",
            "raw_before_parse_requirement": True,
            "retry_policy": "no automatic retry for success or failure within Batch 002",
    ...
    *,
    ...
    if dispatcher is None:
        try:
            ...
            decision = {
                "execution_status": "ATTENTION_BATCH_002_BLOCKED",
    ...
    decision = {
        "execution_status": "ATTENTION_BATCH_002_COMPLETE" if successful == EXPECTED_CALL_COUNT else "ATTENTION_BATCH_002_PARTIALLY_COMPLETE",
