...
function apiFetchGovernedHistoricalUsdJpyObservation(params) {
  ...
  for (var i = 0; i < HISTORICAL_USDJPY_ENDPOINT_PROVIDERS.length; i++) {
    var provider = HISTORICAL_USDJPY_ENDPOINT_PROVIDERS[i];
    var attempt = _historicalUsdJpyFetchProvider_(provider, request, properties);
    attempts.push(attempt);
    if (attempt.status === 'SUCCESS') {
      return _historicalUsdJpyResponse_(request, attempts, attempt, 'SUCCESS', '');
    }
  }

  return _historicalUsdJpyResponse_(request, attempts, null, 'MISSING_OR_UNUSABLE', _historicalUsdJpyFinalReason_(attempts));
}
...
function _historicalUsdJpyResponse_(request, attempts, selected, status, reason) {
