Blueprint ver.1.3.1 — Economic Event AI Prediction System (4th, Feb 2026)
Code-authoritative specification.

1) Purpose & Scope (Blueprint ver.1.3.1)
PreSignal ver.1.3.1 is a Google Sheets + Apps Script system that:
...
Fetches released actual values for events and writes them back into the Event sheet (released_value / released_ts / provider metadata), using SeriesMap-based resolution and provider fetch logic.
...
Entities
EconomicEvent A single macroeconomic release row stored in the Event sheet (canonical). The system expects, at minimum, that the Event sheet exists and contains recognizable headers for event metadata and timestamps. Ingestion via the FMP calendar module will enforce the Event header contract (below) by appending any missing columns to the end of the header row.
Prediction A single AI output row stored in the Predictions sheet. Each row represents one model run for one event (or a synthetic batch summary, depending on runner behavior). The prediction runner enforces the Predictions header contract (below) by appending any missing columns to the end of the header row before writing outputs.
LogRecord A single append-only log row stored in the log sheet with structured JSON context. Logging is “best effort,” and relies on the log sheet already existing.
SeriesMap / SeriesMap Proposals / SeriesMap Suggestions Mapping and triage entities used by the actuals resolver and the SeriesMap workflow:
SeriesMap: mapping table used to resolve provider series IDs for events.
SeriesMap_Proposals: human triage queue (default mode is manual triage in code).
SeriesMap_Suggestions: generated candidate suggestions.
...
3) Identity, Batching, and Synthetic Rows (Blueprint ver.1.3.1)
...
If the Event sheet is missing any of these columns: country, indicator_name, release_ts, event_id, batch_id, type the function throws an error and does not proceed.
...
3.6 Synthetic “batch summary” rows in Predictions
When the prediction runner generates predictions, it contains special behavior for batched events:
If an event has:
type === "member" and
a non-empty batch_id
Then the runner ensures there is a synthetic “batch” row in the Predictions sheet.
This synthetic row is created by _ensureSyntheticBatchRow_() with these key properties:
event_id = the batch_id (note: not a real Event row ID)
type = "batch"
ai_name = "BatchSynth"
ai_model = "BatchAggregator"
qualitative_only = "true" (string)
expected_move_pips_min/max are set from CFG.PIPS_BY_IMPORTANCE based on the member event’s importance
It is only appended if a matching row does not already exist (same event_id == batch_id and type == "batch")
This synthetic row is a placeholder summary row and does not represent an AI call for an actual economic release; it exists to make batch-level handling explicit inside the Predictions table.


4) Event Ingestion & Upsert (Blueprint ver.1.3.1)
4.1 Source & responsibility
Event ingestion in ver.1.3.1 is implemented only via the FMP economic calendar pipeline in fmp_calendar.gs.
...
object → "econ_event"
indicator_name → first non-empty of: indicator_name | title | event | name | category
country → first non-empty of: country | ccy | region, uppercased
genre → first non-empty of: genre | category | group
importance → first non-empty of: importance | impact | importanceText | importance_level
Consensus / previous:
consensus_value pulls first non-empty from: consensus_value | consensus | estimate | forecast | expected
prev_revision pulls first non-empty from: prev_revision | previous | previousValue | prev | prior | revised | revisedPrevious
...
release_ts is derived from first non-empty of: release_ts | datetime | date | time
...
Important: released fields are currently not populated by normalization Although the normalizer contains local variables for “released value / ts,” the returned object currently sets:
released_value: ''
released_ts: ''
source_provider: ''
source_series_id: ''
...
In-memory sort before write runFmpUpcomingToEvent_() sorts normalized rows by release_ts ascending (lexicographic on ISO string).
Sheet sort after write _upsertEventsToEvent_() attempts to sort the Event sheet range (rows 2..last) by the release_ts column ascending.
...
batch_id = ''
Therefore:
✅ Upsert updates will overwrite the entire row, and will blank out type/event_id/batch_id for any matched rows. ➡️ This is expected because the menu flow immediately runs applyBatchingForKeys_() afterward to repopulate those identity fields deterministically.
...
Excellent. Below is the fully rewritten Section 5) aligned with:
...
5) Prediction Runner & Providers (Blueprint ver.1.3.1 — Revised with Feature Pack)
...
From Menu → PreSignal v1.3 → ② Predictions:
...
If no providers enabled: Return {status:'validation_error', message:'No providers enabled'}
...
qualitative_result defaults: parsed valid OR inferred OR inline
expected_move_dir defaults: parsed OR derived OR flat
Pips band defaults by importance
holding minutes default = 60
fallback direction rule: flat if max ≤ 1, else up
...
status
Upsert key: (event_id, ai_name)
Overwrite if exists, else append.

5.12 Synthetic Batch Rows
If:
type == "member"
batch_id non-empty
Ensure synthetic batch row exists:
event_id = batch_id
type = "batch"
ai_name = "BatchSynth"
ai_model = "BatchAggregator"
qualitative_only = true
Pips band from importance.
Created only if not already present.

6) Actuals Fetching & Status Lifecycle (Blueprint ver.1.3.1)
6.1 Purpose
The Actuals Fetcher scans Event rows in a rolling time window, resolves a SeriesMap mapping for each eligible event, fetches the latest available observation (FRED-first), optionally applies a transform, and writes the results back into the Event sheet:
released_value
released_ts
source_provider
source_series_id
...
It then tries to resolve a SeriesMap mapping using whichever resolver exists:
Preferred:
...
If map.notes matches /synthetic batch event/i then the event is skipped with an info log:
...
6.7 Provider priority (FRED-first, FMP fallback)
...
Then ACTUALS_CFG.SOURCE_PRIORITY appended without duplicates Default: ['FRED','FMP']
...
7) SeriesMap: Structure, Resolution, and Suggestion Workflow (Blueprint ver.1.3.1)
7.1 Purpose
...
Regex literal Format: /.../flags
If no i flag is included, i is automatically added.
Regex is applied to the normalized indicator string.
Text pattern (substring match) Any other value is treated as case-insensitive substring match against the normalized indicator string.
...
Candidate columns (appended on the right):
...
If the sheet already exists, _ensureSuggestionsSheet_() will append any missing candidate headers to the right (no reordering).
...
7.8 Suggestion generation sources (heuristics + FMP candidates)
When a suggestion row is appended:
Minimal heuristics from name (suggestFromName_())
Guesses:
freq from keywords (weekly/jobless→W, GDP/quarter→Q, CPI/payroll/etc→M, else IRREGULAR)
unit_type and transform from keywords:
yoy → percent_1dp + yoy
mom → percent_1dp + mom
change/diff → raw + diff
speech/minutes/testimony → qualitative + skip
otherwise → raw + level
precision_dp based on unit/transform
Defaults enforcement (_fillSeriesMapDefaults_())
Ensures:
missing freq → IRREGULAR (policy default)
missing unit_type → raw
missing transform → level
missing precision_dp is computed
notes gets REVIEW: missing provider/series_id if provider/series_id are missing
FMP calendar candidate augmentation (_augmentSuggestionWithFmp_())
Searches FMP economic calendar around release_ts (±1 day) and suggests “calendar series IDs” of the form:
calendar:<FMP event string>
Candidates are stored:
In notes as a breadcrumb: FMP_CANDIDATES: calendar:... (score) | ...
In candidate slots but never cand_1
Policy: cand_1 is reserved for FRED primary, so FMP candidates write into cand_2 first, then cand_3.

7.9 Building suggestions from Event rows (windowed builder)
Window builder
buildSeriesMapSuggestionsWindow_(startIso, endIso):
Reads Event sheet rows where:
release_ts is within [startIso, endIso) (string compare on ISO-Z)
For each unique (country|indicator_name) within that window:
calls appendSeriesMapSuggestion_(country, indicator_name, release_ts)
De-dupes within the run by (country|indicator_name) (not by pattern)
If Event is missing required headers:
throws: Event missing required headers: country, indicator_name, release_ts
Canonical menu entrypoint (window resolution)
buildSeriesMapSuggestionsUsingWindow_() resolves window as:
If resolveWindow_('seriesmap_suggest') exists and returns enabled fromUtcIso/toUtcIso:
uses that window
Else fallback:
from = now (UTC, minute precision)
to = now + 72 hours
note: fallback:next72h (focused on upcoming releases)
It displays a toast summarizing scanned/built/skipped.
...
7.11 FRED Auto-Suggest (writes cand_1 from FRED search)
This is a separate, operator-driven tool in seriesmap_fred_autosuggest.gs.
What it does
Operates only on selected rows in SeriesMap_Suggestions
Queries the FRED “series search” endpoint
Writes ranked results into candidate columns:
cand_1_*, cand_2_*, cand_3_*
Writes classification metadata:
...
FRED_API_KEY If missing, it logs an error and shows an alert.
...
8) Actuals Backfill & Historical Update Mechanics (Blueprint ver.1.3.1)
8.1 Purpose
In addition to the rolling-window Actuals Harvester (Section 6), ver.1.3.1 contains a one-off, ignore-window backfill utility designed to:
...
Primary: resolveSeriesForEvent(evObj, mapRows)
Uses loadSeriesMap() output loaded once at the beginning of the run.
Fallback: _autoMapIndicator(evObj)
...
9) Market Reaction Scoring (Blueprint ver.1.3.1) — REWRITTEN (code-authoritative)
...
p1 = h.close
diff = p1 - p0
pips = round((diff * 100), 2) (USD/JPY: 0.01 JPY = 1 pip → multiply by 100; rounded to 2 decimals)
Direction flag:
dir = 1 if pips > 0
dir = -1 if pips < 0
dir = 0 if pips == 0
...
10) Logging, Error Handling, and Operational Guarantees (Blueprint ver.1.3.1)
10.1 Logging architecture (two-layer design)
Logging is implemented as:
Low-level log writer (always-load shim) — 00_logging_shim.gs Provides:
...
Module wrapper logger(s) — primarily in actuals_fetcher.gs Provides:
...
Calls like appendLog('info', 'message', ctx) do not write (the “sheet” is a string). In these cases, logging effectively becomes silent / dropped, because the shim catches errors and avoids throwing.
In ver.1.3.1 codebase, both styles exist:
...
Given the code behavior, ver.1.3.1 guarantees:
...
11) Menus, Triggers, and Control-Plane Operations (Blueprint ver.1.3.1) — REWRITTEN (code-authoritative)
...
Build SeriesMap Suggestions (Window / next 72h fallback) → menuSeriesMapBuildSuggestions_()
Delegates to buildSeriesMapSuggestionsUsingWindow_() and logs the result
SeriesMap → Auto-suggest from FRED (Selected rows) → menuSeriesMapAutoSuggestFRED_()
Delegates to the FRED autosuggest tool for selected rows on SeriesMap_Suggestions
...
The scheduled automation path is: trigger → menuActualsManualFetch_() → runFetchActualsWindow_(...).
...
12) Configuration & Constants (Blueprint ver.1.3.1)
12.1 Authoritative configuration sources (precedence)
In ver.1.3.1, configuration values are pulled from three places. Precedence depends on the module:
...
The following keys are defined in Code.gs and are authoritative defaults for ver.1.3.1:
Sheet names
...
CFG.SCHEMA_VERSION = '1.3'
CFG.RULE_VERSION = '1.3'
...
If you want, I can rewrite Section 9 and Section 11 to match the code precisely before we continue beyond Section 12.



13) System Execution Flow (End-to-End) (Blueprint ver.1.3.1)
...
Trigger: Menu → ① Events → Fetch & Upsert (next 72h) Handler: menuUpsertNext72h_()
What it does:
...
Trigger: Automatically performed at end of ingestion (Step 1) Worker: applyBatchingForKeys_()
What it does:
...
13.4 Step 3 — SeriesMap maintenance (manual but required for actuals)
Actuals fetching depends on SeriesMap. If SeriesMap does not contain a mapping for (country + indicator_name), actuals runs will mostly skip and (in the window harvester) may append suggestions.
Canonical manual workflow:
Build suggestions Menu → ① Events → Build SeriesMap Suggestions (Window / next 72h fallback) Handler: menuSeriesMapBuildSuggestions_() → buildSeriesMapSuggestionsUsingWindow_()
Auto-suggest FRED candidates (optional but primary for FRED-first actuals) Menu → ① Events → SeriesMap → Auto-suggest from FRED (Selected rows) Handler: menuSeriesMapAutoSuggestFRED_()
Promote selected suggestions into SeriesMap Menu → ① Events → Promote Selected Suggestions → SeriesMap Handler: menuSeriesMapPromoteSelected_() → promoteSelectedSeriesMapSuggestions_()
Outputs:
SeriesMap_Suggestions updated/created
SeriesMap appended with operator-approved mappings

13.5 Step 4 — Predictions generation (Event → Predictions)
Trigger: Menu → ② Predictions → Run Predictions (All Providers) or provider-specific manual runs Handler: runPredictionsAll_() (or menuRunPredictionsGemini_/OpenAI_/Claude_())
Preconditions:
...
ensures a synthetic "batch" row exists in Predictions with ai_name='BatchSynth'
...
Trigger (manual): Menu → ③ Actuals → Fetch Actuals (Manual) Trigger (automation): Menu → ③ Actuals → Start Hourly Actuals Fetch (creates trigger that calls menuActualsManualFetch_() hourly)
...
Resolves SeriesMap mapping
Fetches actuals using provider priority (FRED-first; FMP fallback only when series_id is calendar:<...>)
...
Trigger: Menu → ⑤ Maintenance → Backfill Missing Actuals Worker: fetchActualsIgnoreWindowOnce() in actual_backfill.gs
What it does:
...
Trigger (manual): Menu → ④ Market Reaction → Score Market Reaction (past 24h) Handler: scoreMarketReactionPast24h_()
Trigger (config window): Menu → ④ Market Reaction → Score Market Reaction (Config Window) Handler: scoreMarketReactionByConfigWindow_()
What it does:
...
14) Data Contracts: Required Columns, Required Types, and Validation Rules (Blueprint ver.1.3.1)
...
A) Batching post-pass (applyBatchingForKeys_()) Requires these headers to exist:
country
indicator_name
release_ts
event_id
...
B) Prediction Runner (event selection) Requires these headers to exist to meaningfully run:
...
C) Actuals harvester (runFetchActualsWindow_()) Reads:
...
D) Market reaction scoring (scoreMarketReactionPast24h_()) Requires at least one of:
released_ts (preferred) OR release_ts (fallback)
Important type requirement (code-authoritative):
In scoreMarketReactionPast24h_(), timestamps are only accepted if the cell is a Date object (not a string). In scoreMarketReactionByConfigWindow_(), string timestamps can be parsed.
...
schema_version: string (default 1.3)
event_id: string (UUID-like)
type: one of:
'single' | 'member' | 'batch'
ai_name: string (e.g., Gemini, OpenAI, Anthropic, BatchSynth)
...
qualitative_only: stored as a string ("true" or blank) in synthetic rows; for provider rows it may be boolean-to-string depending on normalization
...
15) Provider Integrations: API Calls, Payloads, and Rate/Quota Behaviors (Blueprint ver.1.3.1)
...
Actuals fallback only when SeriesMap uses an explicit calendar:<event name> mapping
FRED (St. Louis Fed)
...
In _fmpFetchActual_():
series_id must match the pattern: calendar:<Event Name>
...
16) Windowing, Timezones, and Timestamp Parsing (Blueprint ver.1.3.1)
...
17) Known Gaps, Broken Links, and Code-Authoritative Limitations (Blueprint ver.1.3.1)
This section enumerates limitations that are explicitly true in the uploaded .gs files. These are not “future ideas” or “possible enhancements”—they are current ver.1.3.1 realities.
...
getFxCandlesForWindow_(pair, releaseTsUtc, preMin, postMin)
This function is not present in any uploaded .gs file. Therefore, Market Reaction scoring will throw at runtime unless your Apps Script project contains an additional file providing this function.
...
But runPredictionsWindow does not exist in the uploaded code. The implemented function is runPredictionsUsingWindow_(), but it is not wired.
Effect:
...
These are not automatically reconciled. As a result, Event rows may carry “maintenance statuses” that do not align with the rolling harvester’s lifecycle semantics.
...
Effect:
In practice, “actuals” is FRED-first. FMP is a narrow fallback.
...
Effect:
ver.1.3.1 is a pipeline + logging system, not a full closed-loop evaluation engine.


18) Operational Playbooks (Blueprint ver.1.3.1)
...
FRED_API_KEY (required for FRED actuals and FRED autosuggest)
...
Run once, manually:
Fetch & Upsert (next 72h) Confirms: FMP access + Event header enforcement + batching post-pass works
Then optionally:
Build SeriesMap Suggestions
AutoSuggest FRED on some suggestions

18.2 Daily “normal operations” runbook (manual, no automation)
A typical daily flow:
Events refresh
Menu → ① Events → Fetch & Upsert (next 72h)
SeriesMap triage (only if new indicators appear)
Menu → ① Events → Build SeriesMap Suggestions
In SeriesMap_Suggestions, select new rows → AutoSuggest FRED
Fill/confirm provider + series_id → Promote Selected Suggestions → SeriesMap
Run predictions
Menu → ② Predictions → Run Predictions (All Providers) (Avoid “Config Window” until wiring is fixed.)
Fetch actuals
Menu → ③ Actuals → Fetch Actuals (Manual)
(Optional) Market reaction
If candle fetcher exists:
Menu → ④ Market Reaction → Score Market Reaction (Config Window) (More robust if timestamps are strings.)
...
If SeriesMap is not maintained, the trigger will run but many rows will skip due to “No SeriesMap match.”

18.4 Handling “new indicator appears” (SeriesMap workflow playbook)
When Event ingestion introduces new (country, indicator_name) pairs:
Build suggestions
Build SeriesMap Suggestions (window)
Open SeriesMap_Suggestions
Filter/sort by notes containing REVIEW
For each new row:
Select the row(s)
Run AutoSuggest FRED
Copy the best cand_* into:
...
Check SeriesMap exists and has a match
Confirm:
...
Confirm you are not expecting FMP generalized series
Rolling actuals only uses FMP when series_id is calendar:<event name>.
...
19) Appendix: Function Index (Blueprint ver.1.3.1)
...
20) Version Notes, Superseded Sections, and Document Integrity (Blueprint ver.1.3.1)
20.1 Version label and authority statement
This document is Blueprint ver.1.3.1.
Authoritative source of truth: the uploaded Apps Script .gs files. If any discrepancy exists between this Blueprint and the code, the code wins.

20.2 Superseded sections (explicit replacements inside ver.1.3.1)
The following sections were rewritten after direct code verification and must be treated as the canonical content for ver.1.3.1:
...
Market Reaction requires getFxCandlesForWindow_() (not included in uploaded code set). Without it, market scoring will throw at runtime.
...
20.4 Scope boundary of ver.1.3.1
Blueprint ver.1.3.1 documents only what is implemented now:
Included:
FMP calendar ingestion → Event
Batching / identity fill
Predictions runner (Gemini/OpenAI/Anthropic)
Actuals fetching (FRED-first, limited FMP fallback)
SeriesMap workflow (suggestions + FRED autosuggest + promotion)
