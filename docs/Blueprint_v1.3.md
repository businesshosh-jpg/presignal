...
#### Prediction
A single AI output row stored in the Predictions sheet. Each row represents one model run for one event (or a synthetic batch summary, depending on runner behavior). The prediction runner enforces the Predictions header contract (below) by appending any missing columns to the end of the header row before writing outputs.
